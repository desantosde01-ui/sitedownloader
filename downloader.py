...
class WebsiteDownloader:
    ...
    def _process_css(self, css_content, css_url, _depth=0):
        """Process CSS: resolve @import rules (recursively) then rewrite url() references"""
        if _depth > 8:
            return css_content
        ...
        def import_replacer(match):
            full_match = match.group(0)
            # Group 1: url(...) form, Group 2: quoted form
            import_url = (match.group(1) or match.group(2) or '').strip()
            media_query = match.group(3)  # e.g. " screen", " print", ""
            ...
            if abs_url in self.resource_cache:
                local_name = os.path.basename(self.resource_cache[abs_url])
                return f'@import "{local_name}"{media_query};'

            # Fetch the imported CSS (network capture first, then fallback)
            imported_css = None
            if abs_url in self.network_resources:
                try:
                    imported_css = self.network_resources[abs_url]['body'].decode('utf-8', errors='ignore')
                except Exception:
                    pass

            if not imported_css and self.session:
                try:
                    resp = self.session.get(abs_url, timeout=15, verify=False)
                    if resp.status_code == 200:
                        imported_css = resp.text
                except Exception:
                    pass
            ...
            # Recursively process the imported CSS before saving
            processed = self._process_css(imported_css, abs_url, _depth + 1)
            local_path = self._save_resource(abs_url, processed.encode('utf-8'), 'text/css')
            if not local_path:
                return full_match

            local_name = os.path.basename(local_path)
            self.log(f"   📥 @import resolvido: {import_url.split('/')[-1]}")
            return f'@import "{local_name}"{media_query};'
    ...
    def process(self):
        ...
        for link in soup.find_all('link', rel='stylesheet'):
            ...
            if css_content:
                css_content = self._process_css(css_content, abs_url)
        ...
        for style_tag in soup.find_all('style'):
            if style_tag.string:
                style_tag.string = self._process_css(style_tag.string, self.base_url)
